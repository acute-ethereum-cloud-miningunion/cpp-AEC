...
#define aec_SIMPLE_EXCEPTION_VM(X) struct X: VMException { const char* what() const noexcept override { return #X; } }
aec_SIMPLE_EXCEPTION_VM(BadInstruction);
aec_SIMPLE_EXCEPTION_VM(BadJumpDestination);
aec_SIMPLE_EXCEPTION_VM(OutOfGas);
aec_SIMPLE_EXCEPTION_VM(OutOfStack);
aec_SIMPLE_EXCEPTION_VM(StackUnderflow);
aec_SIMPLE_EXCEPTION_VM(DisallowedStateChange);
aec_SIMPLE_EXCEPTION_VM(BufferOverrun);
