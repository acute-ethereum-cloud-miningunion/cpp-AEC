...
{
    return new (std::nothrow) dev::aec::VM;
}
...
{
    delete static_cast<dev::aec::VM*>(_instance);
}
...
{
    auto vm = static_cast<dev::aec::VM*>(_instance);
    evmc_result result = {};
    dev::aec::owning_bytes_ref output;
    ...
    catch (dev::aec::RevertInstruction& ex)
    {
        result.status_code = EVMC_REVERT;
        result.gas_left = vm->m_io_gas;
        output = ex.output();  // This moves the output from the exception!
    }
    catch (dev::aec::VMException const&)
    {
...
VM::VM()
  : evmc_instance{
        EVMC_ABI_VERSION,
        "interpreter",
        alaec_get_buildinfo()->project_version,
        ::destroy,
        ::execute,
        nullptr,
    }
    ...
    DO_CASES
    {
        ...
        CASE(SUICIDE)
        {
            ...
            if (balance > 0 || m_rev < EVMC_SPURIOUS_DRAGON)
            {
                ...
                // aecers to non-existing account.
