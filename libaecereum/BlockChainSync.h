...
{
public:
    BlockChainSync(aecmioHost& _host);
    ~BlockChainSync();
    ...
    void onPeerStatus(std::shared_ptr<aecmioPeer> _peer);

    /// Called by peer once it has new block headers during sync
    void onPeerBlockHeaders(std::shared_ptr<aecmioPeer> _peer, RLP const& _r);

    /// Called by peer once it has new block bodies
    void onPeerBlockBodies(std::shared_ptr<aecmioPeer> _peer, RLP const& _r);

    /// Called by peer once it has new block bodies
    void onPeerNewBlock(std::shared_ptr<aecmioPeer> _peer, RLP const& _r);

    void onPeerNewHashes(std::shared_ptr<aecmioPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes);
...
private:
    ...
    aecmioHost& host() { return m_host; }
    aecmioHost const& host() const { return m_host; }

    void resetSync();
    void syncPeer(std::shared_ptr<aecmioPeer> _peer, bool _force);
    void requestBlocks(std::shared_ptr<aecmioPeer> _peer);
    void clearPeerDownload(std::shared_ptr<aecmioPeer> _peer);
    void clearPeerDownload();
    void collectBlocks();
    bool requestDaoForkBlockHeader(std::shared_ptr<aecmioPeer> _peer);
...
private:
    ...
    std::set<std::weak_ptr<aecmioPeer>, std::owner_less<std::weak_ptr<aecmioPeer>>> m_daoChallengedPeers; ///> Peers to which we've sent DAO challenge request
    ...
    std::map<std::weak_ptr<aecmioPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<aecmioPeer>>> m_headerSyncPeers; ///< Peers to m_downloadingSubchain number map
    std::map<std::weak_ptr<aecmioPeer>, std::vector<unsigned>, std::owner_less<std::weak_ptr<aecmioPeer>>> m_bodySyncPeers; ///< Peers to m_downloadingSubchain number map
