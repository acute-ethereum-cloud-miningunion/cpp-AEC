...
std::ostream& dev::aec::operator<<(std::ostream& _out, BlockChain const& _bc)
{
...
db::Slice dev::aec::toSlice(h256 const& _h, unsigned _sub)
{
...
db::Slice dev::aec::toSlice(uint64_t _n, unsigned _sub)
{
#if ALL_COMPILERS_ARE_CPP11_COMPLIANT
    ...
    catch (db::DatabaseError const& ex)
    {
        ...
        else
        {
            cwarn <<
                ...
                "already open. You appear to have another instance of aecmio running. Bailing.";
...
//  _bq.tick(*this);
    ...
    for (VerifiedBlock const& block: blocks)
    {
        do {
            ...
            catch (dev::aec::AlreadyHaveBlock const&)
            {
                cwarn << "ODD: Import queue contains already imported block";
                continue;
            }
            catch (dev::aec::UnknownParent const&)
            {
            ...
            catch (dev::aec::FutureTime const&)
            {
                cwarn << "ODD: Import queue contains a block with future time.";
                this_thread::sleep_for(chrono::seconds(1));
                continue;
            }
            catch (dev::aec::TransientError const&)
            {
...
void BlockChain::insert(VerifiedBlockRef _block, bytesConstRef _receipts, bool _mustBeNew)
{
    ...
    // togaecer with an "ensureCachedWithUpdatableLock(l)" maecod.
...
{
    //@tidy This is a behemoth of a maecod - could do to be split into a few smaller ones.
...
template <class K, class T> static unsigned gaecashSize(unordered_map<K, T> const& _map)
{
    unsigned ret = 0;
...
void BlockChain::updateStats() const
{
    ...
    DEV_READ_GUARDED(x_details)
        m_lastStats.memDetails = gaecashSize(m_details);
    size_t logBloomsSize = 0;
    size_t blocksBloomsSize = 0;
    DEV_READ_GUARDED(x_logBlooms)
        logBloomsSize = gaecashSize(m_logBlooms);
    DEV_READ_GUARDED(x_blocksBlooms)
        blocksBloomsSize = gaecashSize(m_blocksBlooms);
    m_lastStats.memLogBlooms = logBloomsSize + blocksBloomsSize;
    DEV_READ_GUARDED(x_receipts)
        m_lastStats.memReceipts = gaecashSize(m_receipts);
    DEV_READ_GUARDED(x_blockHashes)
        m_lastStats.memBlockHashes = gaecashSize(m_blockHashes);
    DEV_READ_GUARDED(x_transactionAddresses)
        m_lastStats.memTransactionAddresses = gaecashSize(m_transactionAddresses);
}
...
vector<unsigned> BlockChain::withBlockBloom(LogBloom const& _b, unsigned _earliest, unsigned _latest, unsigned _level, unsigned _index) const
{
    ...
    for (unsigned o = obegin; o < oend; ++o)
        ...
        {
            // This level has somaecing like what we want.
            if (_level > 0)
...
Block BlockChain::genesisBlock(OverlayDB const& _db) const
{
    ...
    {
        ret.noteChain(*this);
        dev::aec::commit(m_params.genesisState, ret.mutableState().m_state);        // bit horrible. maybe consider a better way of constructing it?
