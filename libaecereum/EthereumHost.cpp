...
#include <libaeccore/Exceptions.h>
...
unsigned const aecmioHost::c_oldProtocolVersion = 62; //TODO: remove this once v63+ is common
static unsigned const c_maxSendTransactions = 256;

char const* const aecmioHost::s_stateNames[static_cast<int>(SyncState::Size)] = {"NotSynced", "Idle", "Waiting", "Blocks", "State"};

namespace
{
class aecmioPeerObserver: public aecmioPeerObserverFace
{
public:
    aecmioPeerObserver(shared_ptr<BlockChainSync> _sync, TransactionQueue& _tq): m_sync(_sync), m_tq(_tq) {}

    void onPeerStatus(std::shared_ptr<aecmioPeer> _peer) override
    {
        try
        {
        ...
        {
            // "fix" for https://github.com/aecmio/webthree-umbrella/issues/300
            cwarn << "Failed invariant during sync, restarting sync";
            m_sync->restartSync();
        }
    }

    void onPeerTransactions(std::shared_ptr<aecmioPeer> _peer, RLP const& _r) override
    {
    ...
    void onPeerBlockHeaders(std::shared_ptr<aecmioPeer> _peer, RLP const& _headers) override
    {
        try
        {
    ...
    void onPeerBlockBodies(std::shared_ptr<aecmioPeer> _peer, RLP const& _r) override
    {
        try
        {
    ...
    void onPeerNewHashes(std::shared_ptr<aecmioPeer> _peer, std::vector<std::pair<h256, u256>> const& _hashes) override
    {
        try
        {
    ...
    void onPeerNewBlock(std::shared_ptr<aecmioPeer> _peer, RLP const& _r) override
    {
        try
        {
    ...
    void onPeerNodeData(std::shared_ptr<aecmioPeer> /* _peer */, RLP const& _r) override
    {
        unsigned itemCount = _r.itemCount();
        LOG(m_logger) << "Node Data (" << dec << itemCount << " entries)";
    }

    void onPeerReceipts(std::shared_ptr<aecmioPeer> /* _peer */, RLP const& _r) override
    {
        unsigned itemCount = _r.itemCount();
...
class aecmioHostData: public aecmioHostDataFace
{
public:
    aecmioHostData(BlockChain const& _chain, OverlayDB const& _db): m_chain(_chain), m_db(_db) {}
...
aecmioHost::aecmioHost(BlockChain const& _ch, OverlayDB const& _db, TransactionQueue& _tq, BlockQueue& _bq, u256 _networkId):
    HostCapability<aecmioPeer>(),
    Worker		("aecsync"),
    m_chain		(_ch),
    m_db(_db),
    m_tq		(_tq),
    ...
    m_hostData(make_shared<aecmioHostData>(m_chain, m_db))
{
    // TODO: Composition would be better. Left like that to avoid initialization
    //       issues as BlockChainSync accesses other aecmioHost members.
    m_sync.reset(new BlockChainSync(*this));
    m_peerObserver = make_shared<aecmioPeerObserver>(m_sync, m_tq);
    m_latestBlockSent = _ch.currentHash();
    m_tq.onImport([this](ImportResult _ir, h256 const& _h, h512 const& _nodeId) { onTransactionImported(_ir, _h, _nodeId); });
}

aecmioHost::~aecmioHost()
{
    terminate();
}

bool aecmioHost::ensureInitialised()
{
...
void aecmioHost::completeSync()
{
    m_sync->completeSync();
}

void aecmioHost::doWork()
{
    ...
    {
        m_lastTick = now;
        foreachPeer([](std::shared_ptr<aecmioPeer> _p) { _p->tick(); return true; });
    }
...
void aecmioHost::maintainTransactions()
{
    // Send any new transactions.
    unordered_map<std::shared_ptr<aecmioPeer>, std::vector<size_t>> peerTransactions;
    auto ts = m_tq.topTransactions(c_maxSendTransactions);
    {
        ...
        for (size_t i = 0; i < ts.size(); ++i)
        {
            ...
            auto peers = get<1>(randomSelection(0, [&](aecmioPeer* p) { return p->m_requireTransactions || (unsent && !p->m_knownTransactions.count(t.sha3())); }));
    ...
    foreachPeer([&](shared_ptr<aecmioPeer> _p)
    {
        bytes b;
        unsigned n = 0;
...
void aecmioHost::foreachPeer(std::function<bool(std::shared_ptr<aecmioPeer>)> const& _f) const
{
    ...
    for (auto s: sessions)
        if (!_f(capabilityFromSession<aecmioPeer>(*s.first)))
            return;

    sessions = peerSessions(c_oldProtocolVersion); //TODO: remove once v61+ is common
    std::sort(sessions.begin(), sessions.end(), sessionLess);
    for (auto s: sessions)
        if (!_f(capabilityFromSession<aecmioPeer>(*s.first, c_oldProtocolVersion)))
            return;
}

tuple<vector<shared_ptr<aecmioPeer>>, vector<shared_ptr<aecmioPeer>>, vector<shared_ptr<SessionFace>>> aecmioHost::randomSelection(unsigned _percent, std::function<bool(aecmioPeer*)> const& _allow)
{
    vector<shared_ptr<aecmioPeer>> chosen;
    vector<shared_ptr<aecmioPeer>> allowed;
    vector<shared_ptr<SessionFace>> sessions;

    size_t peerCount = 0;
    foreachPeer([&](std::shared_ptr<aecmioPeer> _p)
    {
        if (_allow(_p.get()))
        {
...
void aecmioHost::maintainBlocks(h256 const& _currentHash)
{
    // Send any new blocks.
    ...
    if (detailsFrom.totalDifficulty < detailsTo.totalDifficulty)
    {
        if (diff(detailsFrom.number, detailsTo.number) < 20)
        {
            ...
            auto s = randomSelection(25, [&](aecmioPeer* p){
                DEV_GUARDED(p->x_knownBlocks)
                    return !p->m_knownBlocks.count(_currentHash);
                return false;
            });
            for (shared_ptr<aecmioPeer> const& p: get<0>(s))
            ...
            for (shared_ptr<aecmioPeer> const& p: get<1>(s))
            {
                RLPStream ts;
...
bool aecmioHost::isSyncing() const
{
    return m_sync->isSyncing();
}

SyncStatus aecmioHost::status() const
{
    return m_sync->status();
}

void aecmioHost::onTransactionImported(ImportResult _ir, h256 const& _h, h512 const& _nodeId)
{
    auto session = host()->peerSession(_nodeId);
    if (!session)
        return;

    std::shared_ptr<aecmioPeer> peer = capabilityFromSession<aecmioPeer>(*session);
    if (!peer)
        peer = capabilityFromSession<aecmioPeer>(*session, c_oldProtocolVersion);
    if (!peer)
        return;
...
shared_ptr<Capability> aecmioHost::newPeerCapability(shared_ptr<SessionFace> const& _s, unsigned _idOffset, p2p::CapDesc const& _cap)
{
    auto ret = HostCapability<aecmioPeer>::newPeerCapability(_s, _idOffset, _cap);

    auto cap = capabilityFromSession<aecmioPeer>(*_s, _cap.second);
    assert(cap);
    cap->init(
        protocolVersion(),
        m_networkId,
