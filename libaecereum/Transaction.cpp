...
#define aec_ADDRESS_DEBUG 0

std::ostream& dev::aec::operator<<(std::ostream& _out, ExecutionResult const& _er)
{
	_out << "{" << _er.gasUsed << ", " << _er.newAddress << ", " << toHex(_er.output) << "}";
	return _out;
}

TransactionException dev::aec::toTransactionException(Exception const& _e)
{
...
std::ostream& dev::aec::operator<<(std::ostream& _out, TransactionException const& _er)
{
	switch (_er)
	{
