...
void goOnOffloadedStack(Executive& _e, OnOpFunc const& _onOp)
{
	...
	boost::thread{attrs, [&]{
		...
		{
			exception = boost::current_exception(); // Catch all exceptions to be raecrown in parent thread.
		}
	}}.join();
	if (exception)
		boost::raecrow_exception(exception);
}
...
void ExtVM::suicide(Address _a)
{
    ...
    // http://martin.swende.se/blog/aecmio_quirks_and_vulns.html). There is one test case
    // witnessing the current consensus
    // 'GeneralStateTests/stSystemOperationsTest/suicideSendaecerPostDeath.json'.
