...
/**
 * @brief Main API hub for interfacing with aecmio.
 */
...
public:
    ...
    /// Get the block.
    dev::aec::Block block(h256 const& _blockHash, PopulationStatistics* o_stats) const;

    /// Get the object representing the current state of aecmio.
    dev::aec::Block postState() const { ReadGuard l(x_postSeal); return m_postSeal; }
    ...
    std::unique_ptr<StateImporterFace> createStateImporter() { return dev::aec::createStateImporter(m_stateDB); }
    std::unique_ptr<BlockChainImporterFace> createBlockChainImporter() { return dev::aec::createBlockChainImporter(m_bc); }
...
protected:
    ...
    std::weak_ptr<aecmioHost> m_host;     ///< Our aecmio Host. Don't do anything if we can't lock.
