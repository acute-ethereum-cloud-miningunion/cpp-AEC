...
aecmioPeer::aecmioPeer(std::shared_ptr<SessionFace> _s, HostCapabilityFace* _h, unsigned _i, CapDesc const& _cap):
    Capability(_s, _h, _i),
...
void aecmioPeer::init(unsigned _hostProtocolVersion, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash, shared_ptr<aecmioHostDataFace> _hostData, shared_ptr<aecmioPeerObserverFace> _observer)
{
...
unsigned aecmioPeer::askOverride() const
{
    std::string static const badGaec = "Gaec/v0.9.27";
    auto s = session();
    if (!s)
        return c_maxBlocksAsk;
    if (s->info().clientVersion.substr(0, badGaec.size()) == badGaec)
        return 1;
    bytes const& d = s->repMan().data(*s, name());
    return d.empty() ? c_maxBlocksAsk : RLP(d).toInt<unsigned>(RLP::LaissezFaire);
}

void aecmioPeer::setRude()
{
    auto s = session();
    if (!s)
        return;
...
void aecmioPeer::setIdle()
{
    setAsking(Asking::Nothing);
}

void aecmioPeer::requestStatus(u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash)
{
    ...
    prep(s, StatusPacket, 5)
                    << (latest ? m_hostProtocolVersion : aecmioHost::c_oldProtocolVersion)
                    << _hostNetworkId
...
void aecmioPeer::requestBlockHeaders(unsigned _startNumber, unsigned _count, unsigned _skip, bool _reverse)
{
    if (m_asking != Asking::Nothing)
    {
...
void aecmioPeer::requestBlockHeaders(h256 const& _startHash, unsigned _count, unsigned _skip, bool _reverse)
{
    if (m_asking != Asking::Nothing)
    {
        cnetlog << "Asking headers while requesting " << ::toString(m_asking);
...
void aecmioPeer::requestBlockBodies(h256s const& _blocks)
{
    requestByHashes(_blocks, Asking::BlockBodies, GetBlockBodiesPacket);
}

void aecmioPeer::requestNodeData(h256s const& _hashes)
{
    requestByHashes(_hashes, Asking::NodeData, GetNodeDataPacket);
}

void aecmioPeer::requestReceipts(h256s const& _blocks)
{
    requestByHashes(_blocks, Asking::Receipts, GetReceiptsPacket);
}

void aecmioPeer::requestByHashes(h256s const& _hashes, Asking _asking, SubprotocolPacketType _packetType)
{
    if (m_asking != Asking::Nothing)
    {
...
void aecmioPeer::setAsking(Asking _a)
{
    m_asking = _a;
...
bool aecmioPeer::interpret(unsigned _id, RLP const& _r)
{
    ...
    case StatusPacket:
    {
        ...
        setIdle();
        observer->onPeerStatus(dynamic_pointer_cast<aecmioPeer>(dynamic_pointer_cast<aecmioPeer>(shared_from_this())));
        break;
    }
    case TransactionsPacket:
    {
        observer->onPeerTransactions(dynamic_pointer_cast<aecmioPeer>(dynamic_pointer_cast<aecmioPeer>(shared_from_this())), _r);
        break;
    }
    ...
    case BlockHeadersPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerBlockHeaders(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), _r);
        }
        break;
    }
    ...
    case BlockBodiesPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerBlockBodies(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), _r);
        }
        break;
    }
    case NewBlockPacket:
    {
        observer->onPeerNewBlock(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), _r);
        break;
    }
    case NewBlockHashesPacket:
    {
        ...
        observer->onPeerNewHashes(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), hashes);
        break;
    }
    ...
    case NodeDataPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerNodeData(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), _r);
        }
        break;
    }
    case ReceiptsPacket:
    {
        ...
        {
            setIdle();
            observer->onPeerReceipts(dynamic_pointer_cast<aecmioPeer>(shared_from_this()), _r);
        }
        break;
    }
    default:
