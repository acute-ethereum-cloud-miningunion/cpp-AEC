...
#include <libaecmio/CodeSizeCache.h>
...
/**
 * Model of an aecmio state, essentially a facade for the trie.
 *
 ...
 * The changelog is managed by savepoint(), rollback() and commit() maecods.
 */
class State
{
    friend class ExtVM;
...
public:
    ...
    /// which uses it. If you have no preexisting database then set BaseState to somaecing other
    ...
    /// Populate the state from the given AccountMap. Just uses dev::aec::commit().
    void populateFrom(AccountMap const& _map);

    /// @returns the set containing all addresses currently in use in aecmio.
    /// @warning This is slowslowslow. Don't use it unless you want to lock the object for seconds or minutes at a time.
    /// @throws InterfaceNotSupported if compiled without aec_FATDB.
    std::unordered_map<Address, u256> addresses() const;

    /// @returns the map with maximum _maxResults elements containing hash->addresses and the next
    /// address hash. This maecod faster then addresses() const;
    ...
    /// @param _commitBehaviour whaecer or not to remove empty accounts during commit.
