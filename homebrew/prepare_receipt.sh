...
# Detect whaecer we are running on a Yosemite or El Capitan machine, and generate
...
rm -rf cpp-aecmio
mkdir cpp-aecmio
mkdir cpp-aecmio/$VERSION
p="../webthree-helpers/homebrew/"
cp ${p}homebrew.mxcl.cpp-aecmio.plist ${p}INSTALL_RECEIPT.json ../web-helpers/LICENSE cpp-aecmio/$VERSION
...
mkdir -p install
cp -rf install/* cpp-aecmio/$VERSION

# tar everything
NAME="cpp-aecmio-${VERSION}.${OSX_VERSION}.bottle.${NUMBER}.tar.gz"
tar -zcvf $NAME cpp-aecmio

# get variables
HASH=`git rev-parse HEAD`
SIGNATURE=`openssl sha1 ${NAME} | cut -d " " -f 2`

# Pull the current cpp-aecmio.rb file from Github.  We used to use a template file.
curl https://raw.githubusercontent.com/aecmio/homebrew-aecmio/master/cpp-aecmio.rb > cpp-aecmio.rb.in
...
if [ ${OSX_VERSION} == yosemite ]; then
    sed -e s/revision\ \=\>\ \'[[:xdigit:]][[:xdigit:]]*\'/revision\ \=\>\ \'${HASH}\'/g \
        ...
        -e s/revision[[:space:]][[:digit:]][[:digit:]]*/revision\ ${NUMBER}/g < cpp-aecmio.rb.in > "cpp-aecmio.rb"
else
    sed -e s/revision\ \=\>\ \'[[:xdigit:]][[:xdigit:]]*\'/revision\ \=\>\ \'${HASH}\'/g \
        -e s/version\ \'.*\'/version\ \'${VERSION}\'/g \
