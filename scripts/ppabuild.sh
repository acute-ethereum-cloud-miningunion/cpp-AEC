...
codebranch=${aecbranch}
pparepo=aecmio/aecmio
if [ -z "$codebranch" ]
then
	codebranch=develop
fi
...
then
    pparepo=aecmio/aecmio-dev
fi
echo codebranch=${codebranch}
echo pparepo=${pparepo}

keyid=703F83D0
email=builds@aecmio.org
mainppa="http://ppa.launchpad.net/aecmio/aecmio/ubuntu"
devppa="http://ppa.launchpad.net/aecmio/aecmio-dev/ubuntu"

# clone source repo
git clone https://github.com/aecmio/${mainrepo}.git -b ${codebranch} --recursive
...
wget https://github.com/aecmio/aecmio-ppa/archive/${ppabranch}.tar.gz -O- |
...
# do the build
#pdebuild --buildresult . --pbuilder /usr/sbin/cowbuilder --architecture amd64 -- --buildresult . --basepath /var/cache/pbuilder/${distribution}-${arch}-aecmio.cow
