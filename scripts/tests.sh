...
#------------------------------------------------------------------------------
# Bash script to execute the cpp-aecmio tests.
#
...
if [[ "$TESTS" == "On" ]]; then
    ...
    $BUILD_ROOT/test/testaec -- --testpath $BUILD_ROOT/../test/jsontests

    # Fill some state tests and make sure the result passes linting
    echo "Running testaec filler tests..."
    $BUILD_ROOT/test/testaec -t GeneralStateTests/stExample -- --filltests --testpath $BUILD_ROOT/../test/jsontests
    $BUILD_ROOT/test/testaec -t 'TransitionTests/bcEIP158ToByzantium' -- --filltests --singletest  ByzantiumTransition --testpath $BUILD_ROOT/../test/jsontests
    ...
    #     $BUILD_ROOT/test/testaec -t "VMTests*,StateTests*" -- --vm jit --testpath $BUILD_ROOT/../test/jsontests
    # fi
