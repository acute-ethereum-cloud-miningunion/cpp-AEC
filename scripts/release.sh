...
#------------------------------------------------------------------------------
# Bash script implementing release flow for cpp-aecmio for Linux and macOS.
#
...
# Copy all the cpp-aecmio executables into a temporary directory prior to ZIP creation

cp bench/bench                         $ZIP_TEMP_DIR
cp aec/aec                             $ZIP_TEMP_DIR
cp aeckey/aeckey                       $ZIP_TEMP_DIR
cp aecvm/aecvm                         $ZIP_TEMP_DIR
cp rlp/rlp                             $ZIP_TEMP_DIR
if [[ "$TESTS" == "On" ]]; then
    cp test/testaec       $ZIP_TEMP_DIR
fi
...
# See https://github.com/aecmio/webthree-umbrella/issues/495.

cp libdevcore/*.$DLL_EXT               $ZIP_TEMP_DIR
cp libdevcrypto/*.$DLL_EXT             $ZIP_TEMP_DIR
cp libaecash/*.$DLL_EXT                $ZIP_TEMP_DIR
cp libaecash-cl/*.$DLL_EXT             $ZIP_TEMP_DIR
cp libaecashseal/*.$DLL_EXT            $ZIP_TEMP_DIR
cp libaeccore/*.$DLL_EXT               $ZIP_TEMP_DIR
cp libaecmio/*.$DLL_EXT              $ZIP_TEMP_DIR
...
zip -j $(pwd)/../cpp-aecmio-develop-$ZIP_SUFFIX.zip $ZIP_TEMP_DIR/*
