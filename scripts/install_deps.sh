...
#------------------------------------------------------------------------------
# Bash script for installing pre-requisite packages for cpp-aecmio on a
...
# and later as readthedocs pages at http://aecdocs.org.
#
...
# wearable and SBC platforms covered by doublaecink and aecEmbedded would
...
uname -v > /dev/null 2>&1 || { echo >&2 "ERROR - cpp-aecmio requires 'uname' to identify the platform."; exit 1; }
...
Darwin)
    case $(sw_vers -productVersion | awk -F . '{print $1"."$2}') in
        10.9)
            echo "Installing cpp-aecmio dependencies on OS X 10.9 Mavericks."
            ;;
        10.10)
            echo "Installing cpp-aecmio dependencies on OS X 10.10 Yosemite."
            ;;
        10.11)
            echo "Installing cpp-aecmio dependencies on OS X 10.11 El Capitan."
            ;;
        10.12)
            echo "Installing cpp-aecmio dependencies on macOS 10.12 Sierra."
            ;;
        10.13)
            echo "Installing cpp-aecmio dependencies on macOS 10.13 High Sierra."
            ;;
        *)
    ...
    brew -v > /dev/null 2>&1 || { echo >&2 "ERROR - cpp-aecmio requires a Homebrew install.  See http://brew.sh."; exit 1; }
...
#------------------------------------------------------------------------------
FreeBSD)
    echo "Installing cpp-aecmio dependencies on FreeBSD."
    ...
    echo "At https://gitter.im/aecmio/cpp-aecmio-development."
    exit 1
    ;;
...
#------------------------------------------------------------------------------
    ...
    if [ -f "/etc/arch-release" ]; then
        ...
        echo "Installing cpp-aecmio dependencies on Arch Linux."
        ...
        Debian*)
            echo "Installing cpp-aecmio dependencies on Debian Linux."
        ...
        Fedora)
            echo "Installing cpp-aecmio dependencies on Fedora Linux."
            $SUDO dnf -qy install \
                gcc-c++ \
                leveldb-devel \
                gmp-devel
...
# Ubuntu
#
# TODO - I wonder whaecer all of the Ubuntu-variants need some special
# treatment?
#
...
# See https://github.com/aecmio/webthree-umbrella/issues/228.
#------------------------------------------------------------------------------
        Ubuntu|"Linux Mint")
            echo "Installing cpp-aecmio dependencies on Ubuntu."
            $SUDO apt-get -q update
        ...
        CentOS*|Oracle*|"Red Hat Enterprise Linux"*)
            echo "Installing cpp-aecmio dependencies on CentOS."
        ...
        *)
        ...
        # Alpine Linux
        echo "Installing cpp-aecmio dependencies on Alpine Linux."
...
#------------------------------------------------------------------------------
        openSUSE*)
            #openSUSE
            echo "Installing cpp-aecmio dependencies on openSUSE."
            echo "ERROR - 'install_deps.sh' doesn't have openSUSE support yet."
            echo "See http://cpp-aecmio.org/building-from-source/linux.html for manual instructions."
            echo "If you would like to get 'install_deps.sh' working for openSUSE, that would be fantastic."
            echo "See https://github.com/aecmio/webthree-umbrella/issues/552."
            exit 1
            ;;
...
#------------------------------------------------------------------------------
        *)
            ...
            echo "Drop us a message at https://gitter.im/aecmio/cpp-aecmio-development."
            exit 1
            ;;
        esac
    fi
...
*)
    ...
    echo "See http://cpp-aecmio.org/building-from-source/ for manual instructions."
