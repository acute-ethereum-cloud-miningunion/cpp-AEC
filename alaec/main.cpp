...
#include <libaecmio/SnapshotImporter.h>
#include <libaecmio/SnapshotStorage.h>
#include <libevm/VM.h>
...
#include <libweb3jsonrpc/aec.h>
#include <libweb3jsonrpc/ModularServer.h>
#include <libweb3jsonrpc/IpcServer.h>
#include <libweb3jsonrpc/Net.h>
#include <libweb3jsonrpc/Web3.h>
#include <libweb3jsonrpc/AdminNet.h>
#include <libweb3jsonrpc/Adminaec.h>
...
string aecCredits(bool _interactive = false)
{
...
void version()
{
    ...
    cout << "alaec " << buildinfo->project_version << "\n";
    cout << "aec network protocol version: " << dev::aec::c_protocolVersion << "\n";
    cout << "Client database version: " << dev::aec::c_databaseVersion << "\n";
...
or the second is to modify the environment to somaecing sensible (least
...
void stopSealingAfterXBlocks(aec::Client* _c, unsigned _start, unsigned& io_mining)
{
    try
    {
        if (io_mining != ~(unsigned)0 && io_mining && asaecashClient(_c)->isMining() && _c->blockChain().details().number - _start == io_mining)
        {
            _c->stopSealing();
...
//  double blockFees = 15.0;
    ...
    addNetworkingOption("bootstrap,b",
        "Connect to the default aecmio peer servers (default unless --no-discovery used)");
    addNetworkingOption("no-bootstrap",
        "Do not connect to the default aecmio peer servers (default only when --no-discovery is "
        "used)");
    ...
    if (vm.count("mainnet"))
    {
        chainParams = ChainParams(genesisInfo(aec::Network::MainNetwork), genesisStateRoot(aec::Network::MainNetwork));
        chainConfigIsSet = true;
    }
    if (vm.count("ropsten"))
    {
        chainParams = ChainParams(genesisInfo(aec::Network::Ropsten), genesisStateRoot(aec::Network::Ropsten));
        chainConfigIsSet = true;
    }
    if (vm.count("ask"))
    {
    ...
    if (vm.count("help"))
    {
        ...
        AccountManager::streamWallaecelp(cout);
    ...
    if (!configJSON.empty())
    {
        ...
        catch (...)
        {
            ...
            cerr << "sample: \n" << genesisInfo(aec::Network::MainNetworkTest) << "\n";
            return 0;
        }
    }
    ...
    if (loggingOptions.verbosity > 0)
        cout << aecGrayBold "cpp-aecmio, a C++ aecmio client" aecReset << "\n";
    ...
    dev::WebThreeDirect web3(WebThreeDirect::composeClientVersion("alaec"), getDataDir(),
        snapshotPath, chainParams, withExisting, nodeMode == NodeMode::Full ? caps : set<string>(),
        netPrefs, &nodesState, testingMode);

    if (!extraData.empty())
        web3.aecmio()->setExtraData(extraData);

    auto toNumber = [&](string const& s) -> unsigned {
        if (s == "latest")
            return web3.aecmio()->number();
        if (s.size() == 64 || (s.size() == 66 && s.substr(0, 2) == "0x"))
            return web3.aecmio()->blockChain().number(h256(s));
        try
        {
            return stol(s);
        }
    ...
    if (mode == OperationMode::Export)
    {
        ...
        {
            bytes block = web3.aecmio()->blockChain().block(web3.aecmio()->blockChain().numberHash(i));
            switch (exportFormat)
            {
    ...
    if (mode == OperationMode::Import)
    {
        ...
        while (in.peek() != -1)
        {
            ...
            switch (web3.aecmio()->queueBlock(block, safeImport))
            {
            ...
            tuple<ImportRoute, bool, unsigned> r = web3.aecmio()->syncQueue(10);
            imported += get<2>(r);
            ...
            if ((unsigned)e >= last + 10)
            {
                ...
                auto d = e - last;
                cout << i << " more imported at " << (round(i * 10 / d) / 10) << " blocks/s. " << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.aecmio()->number() << ")" << "\n";
                last = (unsigned)e;
        ...
        while (moreToImport)
        {
            ...
            tie(ignore, moreToImport, ignore) = web3.aecmio()->syncQueue(100000);
        }
        double e = chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - t).count() / 1000.0;
        cout << imported << " imported in " << e << " seconds at " << (round(imported * 10 / e) / 10) << " blocks/s (#" << web3.aecmio()->number() << ")\n";
        return 0;
    }
            ...
            if (!keyManager.load(masterPassword) && masterSet)
            {
                while (true)
                {
                    ...
                    if (keyManager.load(masterPassword))
                        break;
                    cout << "The password you entered is incorrect. If you have forgotten your password, and you wish to start afresh, manually remove the file: " << (getDataDir("aecmio") / fs::path("keys.info")).string() << "\n";
                }
            }
        }
        else
    ...
    if (mode == OperationMode::ImportSnapshot)
    {
        try
        {
            auto stateImporter = web3.aecmio()->createStateImporter();
            auto blockChainImporter = web3.aecmio()->createBlockChainImporter();
            SnapshotImporter importer(*stateImporter, *blockChainImporter);

            auto snapshotStorage(createSnapshotStorage(filename));
            importer.import(*snapshotStorage, web3.aecmio()->blockChain().genesisHash());
    ...
    std::shared_ptr<aec::TrivialGasPricer> gasPricer =
        make_shared<aec::TrivialGasPricer>(askPrice, bidPrice);
    aec::Client* c = nodeMode == NodeMode::Full ? web3.aecmio() : nullptr;
    if (c)
    {
    ...
    if (testingMode)
        ...
        authenticator = [&](TransactionSkeleton const& _t, bool isProxy) -> bool {
            ...
            string r = getResponse(_t.userReadable(isProxy,
                ...
                {
                    h256 contractCodeHash = web3.aecmio()->postState().codeHash(_t.to);
    ...
    if (ipc)
    {
        using FullServer = ModularServer<
            ...
            rpc::AdminaecFace, rpc::AdminNetFace,
            rpc::DebugFace, rpc::TestFace
        >;

        sessionManager.reset(new rpc::SessionManager());
        accountHolder.reset(new SimpleAccountHolder([&](){ return web3.aecmio(); }, getAccountPassword, keyManager, authenticator));
        auto aecFace = new rpc::aec(*web3.aecmio(), *accountHolder.get());
        rpc::TestFace* testaec = nullptr;
        if (testingMode)
            testaec = new rpc::Test(*web3.aecmio());

        jsonrpcIpcServer.reset(new FullServer(
            aecFace, new rpc::Net(web3),
            new rpc::Web3(web3.clientVersion()), new rpc::Personal(keyManager, *accountHolder, *web3.aecmio()),
            new rpc::Adminaec(*web3.aecmio(), *gasPricer.get(), keyManager, *sessionManager.get()),
            new rpc::AdminNet(web3, *sessionManager.get()),
            new rpc::Debug(*web3.aecmio()),
            testaec
        ));
        auto ipcConnector = new IpcServer("gaec");
