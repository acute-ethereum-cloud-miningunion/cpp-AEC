...
#include <libaeccore/BasicAuthority.h>
...
//
// Bob has already done just that in https://github.com/bobsummerwill/cpp-aecmio/commits/cmake_fixes/aecminer,
// and we should apply those changes back to 'develop'.  It is probably best to defer that cleanup
// until after attempting to backport the Genoil aecminer changes, because they are fairly extensive
...
//
// See https://github.com/bobsummerwill/cpp-aecmio/commit/53af845268b91bc6aa1dab53a6eac675157a072b
// See https://github.com/bobsummerwill/cpp-aecmio/commit/3b9e581d7c04c637ebda18d3d86b5a24d29226f4
//
// More generally, the fact that nearly all of the code for aecminer is actually in the 'MinerAux.h'
...
inline std::string credits()
{
    ...
    out << "cpp-aecmio " << dev::Version << endl
        << "   By cpp-aecmio contributors, (c) 2013-2018." << endl
...
public:
    ...
    {
        if (m_minerType == "cpu")
            aecashCPUMiner::setNumInstances(m_miningThreads);
...
private:
    void doBenchmark(std::string const& _m, unsigned _warmupDuration = 15, unsigned _trialDuration = 3, unsigned _trials = 5)
    {
        ...
        cdebug << aecash::boundary(genesis);

        GenericFarm<aecashProofOfWork> f;
        ...
        f.onSolutionFound([&](aecashProofOfWork::Solution) { return false; });

        string platformInfo = aecashCPUMiner::platformInfo();
