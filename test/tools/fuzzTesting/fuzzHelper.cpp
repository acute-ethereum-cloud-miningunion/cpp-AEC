...
using namespace dev;
using namespace std;
const static std::array<aec::Instruction, 47> invalidOpcodes {{
	aec::Instruction::INVALID,
	aec::Instruction::PUSHC,
	aec::Instruction::JUMPC,
	aec::Instruction::JUMPCI,
	aec::Instruction::JUMPTO,
	aec::Instruction::JUMPIF,
	aec::Instruction::JUMPSUB,
	aec::Instruction::JUMPV,
	aec::Instruction::JUMPSUBV,
	aec::Instruction::BEGINSUB,
	aec::Instruction::BEGINDATA,
	aec::Instruction::RETURNSUB,
	aec::Instruction::PUTLOCAL,
	aec::Instruction::GETLOCAL,
	aec::Instruction::XADD,
	aec::Instruction::XMUL,
	aec::Instruction::XSUB,
	aec::Instruction::XDIV,
	aec::Instruction::XSDIV,
	aec::Instruction::XMOD,
	aec::Instruction::XSMOD,
	aec::Instruction::XLT,
	aec::Instruction::XGT,
	aec::Instruction::XSLT,
	aec::Instruction::XSGT,
	aec::Instruction::XEQ,
	aec::Instruction::XISZERO,
	aec::Instruction::XAND,
	aec::Instruction::XOR,
	aec::Instruction::XXOR,
	aec::Instruction::XNOT,
	aec::Instruction::XSHL,
	aec::Instruction::XSHR,
	aec::Instruction::XSAR,
	aec::Instruction::XROL,
	aec::Instruction::XROR,
	aec::Instruction::XPUSH,
	aec::Instruction::XMLOAD,
	aec::Instruction::XMSTORE,
	aec::Instruction::XSLOAD,
	aec::Instruction::XSSTORE,
	aec::Instruction::XVTOWIDE,
	aec::Instruction::XWIDETOV,
	aec::Instruction::XPUT,
	aec::Instruction::XGET,
	aec::Instruction::XSWIZZLE,
	aec::Instruction::XSHUFFLE
}};
...
{
	aec::Instruction inst = (aec::Instruction) _opcode;
	aec::InstructionInfo info = aec::instructionInfo(inst);
	return (info.gasPriceTier != dev::aec::Tier::Invalid && info.name != nullptr
...
std::string RandomCodeBase::generate(int _maxOpNumber, RandomCodeOptions const& _options)
{
	...
	for (auto i = 0; i < size; i++)
	{
		...
		else
		{
			...
			aec::Instruction inst = (aec::Instruction) opcode;
			aec::InstructionInfo info = aec::instructionInfo(inst);
...
std::string RandomCodeBase::fillArguments(aec::Instruction _opcode, RandomCodeOptions const& _options)
{
	aec::InstructionInfo info = aec::instructionInfo(_opcode);
	...
	{
		//PUSH1 ... PUSH32
		if (aec::Instruction::PUSH1 <= _opcode && _opcode <= aec::Instruction::PUSH32)
		{
			code += rndByteSequence(int(_opcode) - int(aec::Instruction::PUSH1) + 1);
			return code;
		}

		//SWAP1 ... SWAP16 || DUP1 ... DUP16
		bool isSWAP = (aec::Instruction::SWAP1 <= _opcode && _opcode <= aec::Instruction::SWAP16);
		bool isDUP = (aec::Instruction::DUP1 <= _opcode && _opcode <= aec::Instruction::DUP16);

		if (isSWAP || isDUP)
		{
			int times = 0;
			if (isSWAP)
				times = int(_opcode) - int(aec::Instruction::SWAP1) + 2;
			else
				times = int(_opcode) - int(aec::Instruction::DUP1) + 1;
		...
		switch (_opcode)
		{
		case aec::Instruction::MSTORE:
			code += getPushCode(rndByteSequence(randomLength32()));	//code
			code += getPushCode(randomSmallMemoryLength());					//index
			return code;
		// case aec::Instruction::RETURNDATASIZE:  // returndatasize takes no args
		case aec::Instruction::RETURNDATACOPY:  //(REVERT memlen1 memlen2)
			...
			return code;
		case aec::Instruction::EXTCODECOPY:
		...
		case aec::Instruction::CALLCODE:
			...
			return code;
		case aec::Instruction::STATICCALL:
		case aec::Instruction::DELEGATECALL:
			//(CALL gaslimit address value memstart1 memlen1 memstart2 memlen2)
			//(CALLCODE gaslimit address value memstart1 memlen1 memstart2 memlen2)
			code += getPushCode(randomSmallMemoryLength());	//memlen2
			code += getPushCode(randomSmallMemoryLength());	//memstart2
			...
			code += getPushCode((unsigned)randomUniInt());	//gaslimit
			return code;
		case aec::Instruction::SUICIDE: //(SUICIDE address)
			code += getPushCode(toString(_options.getRandomAddress()));
			return code;
		case aec::Instruction::RETURN:  //(RETURN memlen1 memlen2)
		case aec::Instruction::REVERT:  //(REVERT memlen1 memlen2)
			code += getPushCode(randomSmallMemoryLength());	//memlen1
			code += getPushCode(randomSmallMemoryLength());	//memlen1
			return code;
		default:
	...
	sendingAddressProbability(3)	// probability of calling to the tx sending account
{
	...
	setWeight(aec::Instruction::STOP, 1);
	for (uint8_t i = static_cast<uint8_t>(aec::Instruction::PUSH1); i <= static_cast<uint8_t>(aec::Instruction::PUSH32); i++)
		setWeight((aec::Instruction) i, 1);
	for (uint8_t i = static_cast<uint8_t>(aec::Instruction::SWAP1); i <= static_cast<uint8_t>(aec::Instruction::SWAP16); i++)
		setWeight((aec::Instruction) i, 10);
	for (uint8_t i = static_cast<uint8_t>(aec::Instruction::DUP1); i <= static_cast<uint8_t>(aec::Instruction::DUP16); i++)
		setWeight((aec::Instruction) i, 10);

	setWeight(aec::Instruction::SIGNEXTEND, 100);
	setWeight(aec::Instruction::ORIGIN, 200);
	setWeight(aec::Instruction::ADDRESS, 200);
	setWeight(aec::Instruction::SLOAD, 200);
	setWeight(aec::Instruction::MLOAD, 200);
	setWeight(aec::Instruction::MSTORE, 400);
	setWeight(aec::Instruction::MSTORE8, 400);
	setWeight(aec::Instruction::SSTORE, 170);
	setWeight(aec::Instruction::CALL, 350);
	setWeight(aec::Instruction::CALLCODE, 170);
	setWeight(aec::Instruction::DELEGATECALL, 300);
	setWeight(aec::Instruction::STATICCALL, 300);
	setWeight(aec::Instruction::CREATE, 350);

	setWeight(aec::Instruction::RETURNDATASIZE, 500);
	setWeight(aec::Instruction::RETURNDATACOPY, 500);
	setWeight(aec::Instruction::REVERT, 500);
...
void RandomCodeOptions::setWeight(aec::Instruction _opCode, int _weight)
{
