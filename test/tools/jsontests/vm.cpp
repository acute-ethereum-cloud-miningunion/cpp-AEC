...
aec::OnOpFunc FakeExtVM::simpleTrace() const
{
    return [](uint64_t steps, uint64_t pc, aec::Instruction inst, bigint newMemSize, bigint gasCost,
               bigint gas, dev::aec::VMFace const* _vm, dev::aec::ExtVMFace const* voidExt) {
...
json_spirit::mValue VmTestSuite::doTests(json_spirit::mValue const& _input, bool _fillin) const
{
    ...
    for (auto& i: _input.get_obj())
    {
        ...
        aec::EnvInfo env = FakeExtVM::importEnv(testInput.at("env").get_obj(), lastBlockHashes);
        FakeExtVM fev(env);
        ...
        if (_fillin)
        {
            ...
            else
            {
                ...
                BOOST_REQUIRE_MESSAGE(testOutput.at("post").type() == obj_type, testname + "fev.exportState returned somaecing not an object.");
        ...
        else
        {
            if (testInput.count("post") > 0)	// No exceptions expected
            {
                ...
                dev::test::FakeExtVM test(aec::EnvInfo{BlockHeader{}, lastBlockHashes, 0});
