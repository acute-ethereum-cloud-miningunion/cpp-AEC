...
*/
/** @file blockchain.cpp
 * @author Christoph Jentzsch <cj@aecdev.com>, Dimitry Khokhlov <dimitry@aecdev.com>
 * @date 2015
...
aec::Network ChainBranch::s_tempBlockchainNetwork = aec::Network::MainNetwork;
aec::Network TestBlockChain::s_sealEngineNetwork = aec::Network::FrontierTest;
json_spirit::mValue BlockchainTestSuite::doTests(json_spirit::mValue const& _input, bool _fillin) const
{
	...
	for (auto const& i : _input.get_obj())
	{
		...
		if (_fillin)
		{
            ...
            set<aec::Network> allnetworks = ImportTest::getAllNetworksFromExpectSections(
...
void overwriteBlockHeaderForTest(mObject const& _blObj, TestBlock& _block, ChainBranch const& _chainBranch)
{
	...
	if (ho.size() != 14)
	{
		...
		if (ho.count("RelTimestamp"))
		{
			...
			tmp.setDifficulty(((const aecash*)sealEngine)->calculateDifficulty(tmp, parentHeader));
		}

		aecash::setMixHash(tmp, ho.count("mixHash") ? h256(ho["mixHash"].get_str()) : aecash::mixHash(header));
		aecash::setNonce(tmp, ho.count("nonce") ? aec::Nonce(ho["nonce"].get_str()) : aecash::nonce(header));
		tmp.noteDirty();
	}
	else
...
void overwriteUncleHeaderForTest(mObject& uncleHeaderObj, TestBlock& uncle, std::vector<TestBlock> const& uncles, ChainBranch const& _chainBranch)
{
	...
	if (uncleHeaderObj.count("populateFromBlock"))
	{
		...
		if (number < importedBlocks.size())
		{
			...
			if (uncleHeaderObj.count("RelTimestamp"))
			{
				...
				uncleHeader.setDifficulty(((const aecash*)sealEngine)->calculateDifficulty(uncleHeader, parentHeader));
	...
	if (overwrite != "false")
	{
		uncleHeader = constructHeader(
			...
			overwrite == "difficulty" ? toInt(uncleHeaderObj.at("difficulty"))
									  :	overwrite == "timestamp" ? ((const aecash*)sealEngine)->calculateDifficulty(uncleHeader, importedBlocks.at((size_t)uncleHeader.number() - 1).blockHeader())
	...
	{
		if (overwrite == "nonce")
			aecash::setNonce(uncleHeader, aec::Nonce(uncleHeaderObj["nonce"].get_str()));
		if (overwrite == "mixHash")
			aecash::setMixHash(uncleHeader, h256(uncleHeaderObj["mixHash"].get_str()));
...
mObject writeBlockHeaderToJson(BlockHeader const& _bi)
{
	...
	o["mixHash"] = toString(aecash::mixHash(_bi));
	o["nonce"] = toString(aecash::nonce(_bi));
...
void checkBlocks(TestBlock const& _blockFromFields, TestBlock const& _blockFromRlp, string const& _testname)
{
	...
	//BOOST_CHECK_MESSAGE(aecash::mixHash(blockHeaderFromFields) == aecash::mixHash(blockFromRlp), _testname + "mixHash in given RLP not matching the block mixHash!");
	//BOOST_CHECK_MESSAGE(aecash::nonce(blockHeaderFromFields) == aecash::nonce(blockFromRlp), _testname + "nonce in given RLP not matching the block nonce!");
