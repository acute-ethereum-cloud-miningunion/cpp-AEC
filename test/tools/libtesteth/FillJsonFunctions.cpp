...
json_spirit::mObject fillJsonWithStateChange(
    State const& _stateOrig, aec::State const& _statePost, aec::ChangeLog const& _changeLog)
{
    ...
    aec::ChangeLog changeLog = _changeLog;
    std::sort(changeLog.begin(), changeLog.end(),
        [](const aec::Change& lhs, const aec::Change& rhs) { return lhs.address < rhs.address; });
    ...
    while (i < changeLog.size())
    {
        ...
        while (i < changeLog.size() && changeLog[i].address == currentAddress)
        {
            ...
            aec::Change change = changeLog.at(i);
            switch (change.kind)
            {
...
json_spirit::mObject fillJsonWithState(State const& _state, aec::AccountMaskMap const& _map)
{
