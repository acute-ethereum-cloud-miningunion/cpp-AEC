...
#include <test/tools/libtestaec/BlockChainHelper.h>
...
ImportTest::ImportTest(json_spirit::mObject const& _input, json_spirit::mObject& _output):
    m_statePre(0, OverlayDB(), aec::BaseState::Empty),
    m_statePost(0, OverlayDB(), aec::BaseState::Empty),
...
void ImportTest::makeBlockchainTestFromStateTest(set<aec::Network> const& _networks) const
{
    ...
    for (auto const& tr : m_transactions)
    {
        ...
        State s = State(0, OverlayDB(), aec::BaseState::Empty);
        AccountMaskMap m;
...
set<aec::Network> ImportTest::getAllNetworksFromExpectSections(
...
void ImportTest::checkBalance(aec::State const& _pre, aec::State const& _post, bigint _miningReward)
{
    bigint preBalance = 0;
    ...
    //account could destroy aecer if it suicides to itself
...
std::tuple<aec::State, ImportTest::ExecOutput, aec::ChangeLog> ImportTest::executeTransaction(aec::Network const _sealEngineNetwork, aec::EnvInfo const& _env, aec::State const& _preState, aec::Transaction const& _tr)
{
    assert(m_envInfo);
    ...
    ExecOutput out(std::make_pair(aec::ExecutionResult(), aec::TransactionReceipt(h256(), u256(), aec::LogEntries())));
    try
    {
        ...
        aec::ChangeLog changeLog = initialState.changeLog();
...
void ImportTest::importTransaction (json_spirit::mObject const& _o, aec::Transaction& o_tr)
{
        ...
        if (network.count(netIdToString(tr.netId)) || network.count("ALL"))
                ...
                (inArray(v, tr.valInd) || v[0] == -1))
            {
                ...
                if (_expects.count("result"))
                {
                    ...
                    aec::AccountMaskMap stateMap;
                    State expectState(0, OverlayDB(), aec::BaseState::Empty);
