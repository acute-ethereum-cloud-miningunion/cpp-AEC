...
#include <test/tools/libtestaec/Stats.h>
...
void TestSuite::executeFile(boost::filesystem::path const& _file) const
{
	...
	BOOST_REQUIRE_MESSAGE(s.length() > 0, "Contents of " << _file.string() << " is empty. Have you cloned the 'tests' repo branch develop and set aecmio_TEST_PATH to its path?");
	json_spirit::read_string(s, v);
	doTests(v, false);
}
