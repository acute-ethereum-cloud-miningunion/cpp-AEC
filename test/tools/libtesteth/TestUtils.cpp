...
{
	m_json = loadJsonFromFile(toTestFilePath(getCommandLineArgument("--aec_testfile")));
}

void ParallelFixture::enumerataecreads(std::function<void()> callback) const
{
	size_t threadsCount = std::stoul(getCommandLineArgument("--aec_threads"), nullptr, 10);
...
void BlockChainFixture::enumerateBlockchains(std::function<void(Json::Value const&, dev::aec::BlockChain const&, State state)> callback) const
{
...
void ClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::aec::ClientBase&)> callback) const
{
...
void ParallelClientBaseFixture::enumerateClients(std::function<void(Json::Value const&, dev::aec::ClientBase&)> callback) const
{
	ClientBaseFixture::enumerateClients([this, &callback](Json::Value const& _json, dev::aec::ClientBase& _client) -> void
	{
		// json is being copied here
		enumerataecreads([callback, _json, &_client]() -> void
		{
