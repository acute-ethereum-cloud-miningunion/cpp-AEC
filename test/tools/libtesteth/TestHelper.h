...
#include <test/tools/libtestaec/ImportTest.h>
#include <test/tools/libtestaec/TestOutputHelper.h>
#include <test/tools/libtestaec/TestSuite.h>
...
class ZeroGasPricer: public aec::GasPricer
{
protected:
	u256 ask(aec::Block const&) const override { return 0; }
	u256 bid(aec::TransactionPriority = aec::TransactionPriority::Medium) const override { return 0; }
};
...
std::string netIdToString(aec::Network _netId);
aec::Network stringToNetId(std::string const& _netname);
bool isDisabledNetwork(aec::Network _net);
std::set<aec::Network> const& getNetworks();
...
aec::LogEntries importLog(json_spirit::mArray const& _o);
std::string exportLog(aec::LogEntries const& _logs);
void checkOutput(bytesConstRef _output, json_spirit::mObject const& _o);
void checkStorage(std::map<u256, u256> _expectedStore, std::map<u256, u256> _resultStore, Address _expectedAddr);
void checkCallCreates(aec::Transactions const& _resultCallCreates, aec::Transactions const& _expectedCallCreates);
dev::aec::BlockHeader constructHeader(
	...
	dev::aec::LogBloom const& _logBloom,
...
void updateaecashSeal(dev::aec::BlockHeader& _header, h256 const& _mixHash, dev::aec::Nonce const& _nonce);
RLPStream createRLPStreamFromTransactionFields(json_spirit::mObject const& _tObj);
json_spirit::mObject fillJsonWithStateChange(aec::State const& _stateOrig, aec::State const& _statePost, aec::ChangeLog const& _changeLog);
json_spirit::mObject fillJsonWithState(aec::State const& _state);
json_spirit::mObject fillJsonWithState(aec::State const& _state, aec::AccountMaskMap const& _map);
json_spirit::mObject fillJsonWithTransaction(aec::Transaction const& _txn);
...
/// This class also provides maecods for registering and notifying the listener
class Listener
{
public:
