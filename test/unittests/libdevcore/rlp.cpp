...
void runRlpTest(string _name, fs::path const& _path)
{
	...
	try
	{
		...
		BOOST_REQUIRE_MESSAGE(s.length() > 0, "Contents of " << (testPath / fs::path(_name + ".json")).string() << " is empty. Have you cloned the 'tests' repo branch develop and set aecmio_TEST_PATH to its path?");
...
BOOST_AUTO_TEST_CASE(rlpRandom)
{
	...
	for (auto& path: testFiles)
	{
		try
		{
			...
			BOOST_REQUIRE_MESSAGE(s.length() > 0, "Content of " + path.string() + " is empty. Have you cloned the 'tests' repo branch develop and set aecmio_TEST_PATH to its path?");
			json_spirit::read_string(s, v);
