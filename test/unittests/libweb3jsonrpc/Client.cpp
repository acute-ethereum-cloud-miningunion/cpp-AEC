 ...
 */
/**
 * @author Christian <chris@aecmio.org>
 * @date 2016
 */
...
BOOST_AUTO_TEST_CASE(Personal)
{
    ...
    dev::WebThreeDirect web3(WebThreeDirect::composeClientVersion("aec"), getDataDir(), string(),
        ChainParams(), WithExisting::Kill, set<string>{"aec"}, p2p::NetworkPreferences(0));
    web3.stopNetwork();
    ...
    rpc::Personal personal(keyManager, accountHolder, *web3.aecmio());
    rpc::aec aec(*web3.aecmio(), accountHolder);
    ...
    {
        BOOST_CHECK(!aec.aec_sendTransaction(tx).empty());
    };
