...
#include <test/tools/libtestaec/TestUtils.h>
...
{
	enumerateClients([](Json::Value const& _json, dev::aec::ClientBase& _client) -> void
	{
		auto compareState = [&_client](Json::Value const& _o, string const& _name, BlockNumber _blockNumber) -> void
		{
			...
			for (string const& pos: _o["storage"].getMemberNames())
			{
				...
				aec_CHECK_EQUAL(expectedState, state);
			}

			// codeAt
			bytes expectedCode = fromHex(_o["code"].asString());
			bytes code = _client.codeAt(address, _blockNumber);
			aec_CHECK_EQUAL_COLLECTIONS(expectedCode.begin(), expectedCode.end(),
										code.begin(), code.end());
		};
		...
		aec_CHECK_EQUAL(expectedNumber, number);
		...
		for (Json::Value const& block: _json["blocks"])
		{
			...
			aec_CHECK_EQUAL(expectedHashFromNumber, hashFromNumber);
			...
			auto compareBlockInfos = [](Json::Value const& _b, BlockHeader _blockInfo) -> void
			{
				...
				aec::Nonce expectedBlockInfoNonce = aec::Nonce(fromHex(_b["nonce"].asString()));
				...
				aec_CHECK_EQUAL(expectedBlockInfoBloom, _blockInfo.logBloom());
				aec_CHECK_EQUAL(expectedBlockInfoCoinbase, _blockInfo.author());
				aec_CHECK_EQUAL(expectedBlockInfoDifficulty, _blockInfo.difficulty());
				aec_CHECK_EQUAL_COLLECTIONS(
					expectedBlockInfoExtraData.begin(),
					expectedBlockInfoExtraData.end(),
					_blockInfo.extraData().begin(),
					_blockInfo.extraData().end()
				);
				aec_CHECK_EQUAL(expectedBlockInfoGasLimit, _blockInfo.gasLimit());
				aec_CHECK_EQUAL(expectedBlockInfoGasUsed, _blockInfo.gasUsed());
				aec_CHECK_EQUAL(expectedBlockInfoHash, _blockInfo.hash());
				aec_CHECK_EQUAL(expectedBlockInfoMixHash, aecash::mixHash(_blockInfo));
				aec_CHECK_EQUAL(expectedBlockInfoNonce, aecash::nonce(_blockInfo));
				aec_CHECK_EQUAL(expectedBlockInfoNumber, _blockInfo.number());
				aec_CHECK_EQUAL(expectedBlockInfoParentHash, _blockInfo.parentHash());
				aec_CHECK_EQUAL(expectedBlockInfoReceiptsRoot, _blockInfo.receiptsRoot());
				aec_CHECK_EQUAL(expectedBlockInfoTimestamp, _blockInfo.timestamp());
				aec_CHECK_EQUAL(expectedBlockInfoTransactionsRoot, _blockInfo.transactionsRoot());
				aec_CHECK_EQUAL(expectedBlockInfoUncldeHash, _blockInfo.sha3Uncles());
			};
			...
			aec_CHECK_EQUAL(expectedBlockDetailsNumber, blockDetails.number);
			aec_CHECK_EQUAL(totalDifficulty, blockDetails.totalDifficulty);
...
//				unsigned expectedTransactionSignatureV = jsToInt(t["v"].asString());
				...
				aec_CHECK_EQUAL(expectedTransactionGasLimit, _transaction.gas());
				aec_CHECK_EQUAL(expectedTransactionGasPrice, _transaction.gasPrice());
				aec_CHECK_EQUAL(expectedTransactionNonce, _transaction.nonce());
				aec_CHECK_EQUAL(expectedTransactionSignatureR, _transaction.signature().r);
				aec_CHECK_EQUAL(expectedTransactionSignatureS, _transaction.signature().s);
//				aec_CHECK_EQUAL(expectedTransactionSignatureV, _transaction.signature().v); // 27 === 0x0, 28 === 0x1, not sure why
			};
			...
			aec_REQUIRE(transactions.size() == ts.size());
			aec_REQUIRE(transactions.size() == tHashes.size());
			
			// transactionCount
			aec_CHECK_EQUAL(transactions.size(), tsCount);
			...
			for (unsigned i = 0; i < tsCount; i++)
			{
				...
				// transactionHashes
				aec_CHECK_EQUAL(transaction.sha3(), tHashes[i]);
			}
			
			// uncleCount
			unsigned usCount = _client.uncleCount(blockHash);
			aec_CHECK_EQUAL(uncles.size(), usCount);
