...
#include <libaecmio/aecmioPeer.h>
...
class MockaecmioPeerObserver: public aecmioPeerObserverFace
{
public:
	void onPeerStatus(std::shared_ptr<aecmioPeer>) override {}

	void onPeerTransactions(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerBlockHeaders(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerBlockBodies(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerNewHashes(std::shared_ptr<aecmioPeer>, std::vector<std::pair<h256, u256>> const&) override {}

	void onPeerNewBlock(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerNodeData(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerReceipts(std::shared_ptr<aecmioPeer>, RLP const&) override {}

	void onPeerAborting() override {}
};

class aecmioPeerTestFixture: public TestOutputHelperFixture
{
public:
	aecmioPeerTestFixture():
		session(std::make_shared<MockSession>()),
		observer(std::make_shared<MockaecmioPeerObserver>()),
		offset(UserPacket),
		peer(session, &hostCap, offset, { "aec", 0 })
	{
		peer.init(63, 2, 0, h256(0), h256(0), std::shared_ptr<aecmioHostDataFace>(), observer);
	}

	MockHostCapability hostCap;
	std::shared_ptr<MockSession> session;
	std::shared_ptr<MockaecmioPeerObserver> observer;
	uint8_t offset;
	aecmioPeer peer;
};

BOOST_FIXTURE_TEST_SUITE(aecmioPeerSuite, aecmioPeerTestFixture)

BOOST_AUTO_TEST_CASE(aecmioPeerSuite_requestNodeData)
{
...
BOOST_AUTO_TEST_CASE(aecmioPeerSuite_requestNodeDataSeveralHashes)
{
...
BOOST_AUTO_TEST_CASE(aecmioPeerSuite_requestNodeDataAddsAskingNote)
{
	h256 dataHash("0x949d991d685738352398dff73219ab19c62c06e6f8ce899fbae755d5127ed1ef");
...
BOOST_AUTO_TEST_CASE(aecmioPeerSuite_requestNodeDataWithNoHashesSetsAskNoteToNothing)
{
...
BOOST_AUTO_TEST_CASE(aecmioPeerSuite_requestReceipts)
{
