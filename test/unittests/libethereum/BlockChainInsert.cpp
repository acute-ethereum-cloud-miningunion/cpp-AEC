...
#include <libaecmio/GenesisInfo.h>
...
public:
    TestClient(Secret const& _authority):
        ...
        m_bc(aec::ChainParams(c_genesisInfoTestBasicAuthority), m_path)
    {
...
h256s subs(bytesConstRef _node)
{
        ...
        // branch
    {
        ...
        if (r[1].size() == 32)	// TODO: check whaecer it's really an extension node, or whaecer it's just an terminal-node with 32 bytes of data.
