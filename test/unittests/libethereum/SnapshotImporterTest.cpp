...
#include <libaecmio/StateImporter.h>
#include <libaecmio/BlockChainImporter.h>
...
BOOST_AUTO_TEST_CASE(SnapshotImporterSuite_importEmptyBlock)
{
	...
	BOOST_CHECK_EQUAL(aecash::mixHash(header), mixHash);
	BOOST_CHECK_EQUAL(aecash::nonce(header), nonce);
