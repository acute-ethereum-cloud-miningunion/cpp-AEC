...
BOOST_FIXTURE_TEST_SUITE(aecashTests, TestOutputHelperFixture)
...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumWithoutUncles)
{
    ...
    BOOST_CHECK_EQUAL(aecash.calculateDifficulty(header, parentHeader), 999024);
}
...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumWithUncles)
{
    ...
    BOOST_CHECK_EQUAL(aecash.calculateDifficulty(header, parentHeader), 999512);
}
...
BOOST_AUTO_TEST_CASE(calculateDifficultyByzantiumMaxAdjustment)
{
    ...
    BOOST_CHECK_EQUAL(aecash.calculateDifficulty(header, parentHeader), 951688);
}
...
BOOST_AUTO_TEST_CASE(epochSeed)
{
    BlockHeader header;
    header.setNumber(0);
    h256 seed = aecash::seedHash(header);
    BOOST_CHECK_EQUAL(seed, h256{});

    header.setNumber(30000);
    seed = aecash::seedHash(header);
    BOOST_CHECK_EQUAL(
...
BOOST_AUTO_TEST_CASE(etashQuickVerify)
{
    ...
    aecash::setMixHash(header, {});

    aecash etash;
    aecash::setNonce(header, Nonce{3272400});
    etash.verify(QuickNonce, header, {}, {});
    aecash::setNonce(header, Nonce{3272401});
...
BOOST_AUTO_TEST_CASE(etashVerify)
{
    ...
    aecash etash;
    aecash::setNonce(header, Nonce{"81c3f9bfae230a8e"});
    etash.verify(CheckEverything, header, {}, {});

    // Break nonce.
    aecash::setNonce(header, Nonce{"71c3f9bfae230a8e"});
    try
    {
        etash.verify(CheckEverything, header, {}, {});
        BOOST_CHECK(false);
    ...
    {
        std::tuple<h256, h256> aecashResult = *boost::get_error_info<errinfo_aecashResult>(ex);
        BOOST_CHECK_EQUAL(std::get<0>(aecashResult),
            h256{"07a4017237d933aa1ff4f62650f68ea2118c8bd741575e97c2867fb41d5b832d"});
        BOOST_CHECK_EQUAL(std::get<1>(aecashResult),
...
aecashTestCase aecashTestCases[] = {
    {
        "4242424242424242",
...
BOOST_AUTO_TEST_CASE(aecashEvalHeader)
{
    // FIXME: Drop this test as aecash library has this test cases in its test suite.

    for (auto& t : aecashTestCases)
    {
        BlockHeader header{fromHex(t.header), HeaderData};
        h256 headerHash{t.headerHash};
        aec::Nonce nonce{t.nonce};
        BOOST_REQUIRE_EQUAL(headerHash, header.hash(WithoutSeal));
        BOOST_REQUIRE_EQUAL(nonce, aecash::nonce(header));

        aecash::result result = aecash::hash(
            aecash::get_global_epoch_context(aecash::get_epoch_number(header.number())),
            aecash::hash256_from_bytes(header.hash(WithoutSeal).data()), (uint64_t)(u64)nonce);
        ...
        BOOST_REQUIRE_EQUAL(mix, aecash::mixHash(header));
    }
}
