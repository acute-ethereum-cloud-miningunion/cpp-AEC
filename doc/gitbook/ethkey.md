...
`aeckey` is a CLI tool that allows you to interact with the aecmio wallet. With it you can list, inspect, create, delete and modify keys and inspect, create and sign transactions.

### Keys and Wallets

When using aecmio you will own one or more `keys`. These are special files that allow you access to a particular account. Such access might allow you to spend funds, register a name or transfer an asset. Keys are standardised and compatible across major clients. They are always protected by password-based encryption. Also they do not directly identify the actual account that the key represents. To determine this, the key must be decrypted through providing the correct password.

In cpp-aecmio, your *wallet* keeps a track of each key that you own along with what *address* it represents. An address is just way of referring to a particular *account* in aecmio. It, too, is protected by a password, which is generally provided when the client begins.
...
We'll assume you have not yet run a client such as `aec` or anything in the Alaec series of clients. If you have, you should skip this section.

To create a wallet, run `aeckey` with the `createwallet` command:
...
One of the nice things about aecmio is that creating a key is tantamount to creating an account. You don't need to tell anybody else you're doing it, you don't even need to be connected to the Internet. Of course your new account will not contain any aecer. But it'll be yours and you can be certain that without your key and your password, nobody else can ever access it.
...
Occasionally, however, it's important to convert between the two. `aeckey` provides the `inspect` command for this purpose. When passed any address, file or UUID, it will tell you information about it including both formats of address.
...
```
> aeckey inspect XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
test (0092e965…)
  ICAP: XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
  Raw hex: 0092e965928626f8880629cec353d3fd7ca5974f
> aeckey inspect 0092e965928626f8880629cec353d3fd7ca5974f
test (0092e965…)
  ICAP: XE472EVKU3CGMJF2YQ0J9RO1Y90BC0LDFZ
  Raw hex: 0092e965928626f8880629cec353d3fd7ca5974f
```
...
Here's an unsigned transaction. It authorises the donation of 1 aecer to me:

```
ec80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801b8080
```

On its own, it won't do much. We can see this by decoding it in `aeckey`:

```
> aeckey decode ec80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801b8080
Transaction 705d490edc318b50223efa7bb9c19d65f05c3c527e4f8e60535b46a2ed128706
  ...
  data: none
  from: <unsigned>
  value: 1 aecer (1000000000000000000 wei)
  nonce: 0
  gas: 21000
...
Note that it states the transaction is `<unsigned>` to the right of `from:`. This means that at present it's useless. Signing it would make it useful (to me, at least, since it'd make me one aecer richer), or dangerous (to you if you didn't want to give me that aecer).

### Signing a Transaction

`aeckey` can be used to sign a pre-existing, but unsigned, transaction (it can also create a transaction and sign it itself). In this case, the transaction is actually harmless anyway since we're signing with the key of a fresh account that has no aecer to be transferred.
...
```
> aeckey sign test ec80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801b8080
...
```
> aeckey decode f86c80850ba43b74008252089400be78bf8a425471eca0cf1d255118bc080abf95880de0b6b3a7640000801ca07638c34170f3e04313bbb6c5bfc10a0c665200515a1aa5e922c7ae6c0dd085faa079ab46048e643bb4042bcb22da86d2646eb0b727f23aa3e165102b824563c70d
...
```
> aeckey import ~/backup-keys/* test
```

or, for Windows:

```
> aeckey import $HOME/backup-keys/*.* test
```
...
### Importing a key from another client (e.g. Gaec)

Because our keys all share the same format it's really easy to import keys from other clients like Gaec. In fact it's exactly the same process as restoring a key from a previous backup as we did in the last step.

If we assume we have a gaec key at `mygaeckey.json`, then to import it to use `aec`, simply use:

```
> aeckey import mygaeckey.json "My Old Gaec Key"
```
...
Security people reckon that it is prudent to change your password regularly. You can do so easily with `aeckey` using the `recode` command (which actually does a whole lot more, but that's advanced usage).
...
There's much more to discover with `aeckey`; it provides a suite of commands for playing with "bare" secrets, those not in the wallet (the `listbare`, `newbare`, ... commands), and allows keys to be imported without actually ever being decrypted (`importwithaddress`) and conversion between ICAP and hex (`inspectbare`).

Options allow you to alter transactions before you sign them and even create transactions from scratch. You can also configure the maecod by which keys are encrypted, changing the encryption function or its parameters.

See `aeckey --help` for more information. Enjoy!
