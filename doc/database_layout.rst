...
cpp-aecmio uses three databases, all of them are essentially just
key-value storages (LevelDB or RocksDB is used depending on build
settings). Their physical disk locations are as follows:

-  Blocks - ``{aec_DATABASE_DIR}/{GENESIS_HASH}/blocks``
-  Extras -
   ``{aec_DATABASE_DIR}/{GENESIS_HASH}/{DATABASE_VERSION}/extras``
-  State -
   ``{aec_DATABASE_DIR}/{GENESIS_HASH}/{DATABASE_VERSION}/state``

where

``{aec_DATABASE_DIR}`` - base cpp-aecmio data directory
...
`BlockChain
class <https://github.com/aecmio/cpp-aecmio/blob/develop/libaecmio/BlockChain.h>`__.
...
-  Records storing log blooms for a number of blocks at once have the
   ...
   maecod <https://github.com/aecmio/cpp-aecmio/blob/db7278413edf701901d2a054b32a31c2722708d5/libaecmio/BlockChain.h#L193-L206>`__
   for details.
...
The data representing the full aecmio state (i.e. all the accounts).
The State data forms a `Merkle Patricia
Trie <https://github.com/aecmio/wiki/wiki/Patricia-Tree>`__ and the
...
class <https://github.com/aecmio/cpp-aecmio/blob/develop/libaecmio/State.h>`__
(also note free function ``commit`` there). Merkle Patricia Trie
implemenation is in
`TrieDB.h <https://github.com/aecmio/cpp-aecmio/blob/develop/libdevcore/TrieDB.h>`__.
For lower-level code accessing the database itself see
`OverlayDB <https://github.com/aecmio/cpp-aecmio/blob/develop/libdevcore/OverlayDB.h>`__
and
`MemoryDB <https://github.com/aecmio/cpp-aecmio/blob/develop/libdevcore/MemoryDB.h>`__.
