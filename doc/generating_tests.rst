...
This article is for testing with the C++ aecmio client. For non-client specific
aecmio testing, refer to http://aecmio-tests.readthedocs.io/. Consensus tests 
are test cases for all aecmio implementations. The test cases are distributed 
...
``testaec`` executable in cpp-aecmio can convert test fillers into test cases.
...
.. _`tests repository`: https://github.com/aecmio/tests
...
::
  ...
  git clone https://github.com/aecmio/tests.git

should create a local copy of the ``develop`` branch of the tests repository. 
From here on, ``<LOCAL_PATH_TO_aec_TESTS>`` points to this local copy.

Preparing testaec and LLL
=========================

For generating consensus tests, an executable ``testaec`` is necessary.  Moreover, 
``testaec`` uses the LLL compiler when it generates consensus tests. The easier way is 
to use the `docker image <https://hub.docker.com/r/holiman/testaec/>`_ provided by 
holiman_ (another `image <https://hub.docker.com/r/winsvega/testaec/>`_ is provided by winsvega_).
...
* `Install Docker`_
* Pull the ``testaec`` repository with ``docker pull holiman/testaec``
* ``docker run -v <LOCAL_PATH_TO_aec_TESTS>:/foobar holiman/testaec -t GeneralStateTests/stCallCodes -- --singletest callcall_00 --singlenet EIP150 -d 0 -g 0 -v 0 --statediff --verbosity 5 --testpath /foobar`` should show somaecing like
...
.. note::
   Some problems with running the ``testaec`` command can be fixed by adding the ``--all`` option
   at the end.
...
Sometimes, you need a tweaked version of ``testaec`` or ``lllc`` when your tests are about very new features not available in the docker image.

``testaec`` is distributed in cpp-aecmio_ and ``lllc`` is distributed in solidity_.  These executable needs to be installed.

.. _cpp-aecmio: https://github.com/aecmio/cpp-aecmio

.. _solidity: https://github.com/aecmio/solidity
...
.. _`an existing test filler`: https://github.com/aecmio/tests/blob/develop/src/GeneralStateTestsFiller/stExample/add11Filler.json
...
.. [#] ``testaec`` cannot understand LLL if the system does not have the LLL compiler installed.  The LLL compiler is currently distributed as part of `the Solidity compiler`_.

.. _`the Solidity compiler`: https://github.com/aecmio/solidity

Writing a Test Filler
---------------------

A new test filler needs to be alone in a new test filler file.  A single GeneralStateTest filler file is not supposed to contain multiple tests.  ``testaec`` tool still accepts multiple GeneralStateTest fillers in a single test filler file, but this might change.
...
.. [#] If you create a new directory here, you need to add `one line`__ in ``cpp-aecmio`` and file that change in a pull-request to ``cpp-aecmio``.
...
.. [#] The file name and the name written in JSON should match because ``testaec`` prints the name written in JSON, but the user needs to find a file.
...
The test filler file is not for consumption.  The filler file needs to be filled into a test.  ``testaec`` has the ability to compute the post-state from the test filler, and produce the test.  The advantage of the filled test is that it can catch any post-state difference between clients.

.. _editcpp:

First, if you created a new subdirectory for the filler, you need to edit the source of ``cpp-aecmio`` so that ``testaec`` recognizes the new subdirectory.  The file to edit is `cpp-aecmio/blob/develop/test/tools/jsontests/StateTests.cpp`_, which lists the names of the subdirectories scanned for GeneralStateTest filters.

.. _`cpp-aecmio/blob/develop/test/tools/jsontests/StateTests.cpp`: https://github.com/aecmio/cpp-aecmio/blob/develop/test/tools/jsontests/StateTests.cpp

After building ``testaec``, you are ready to fill the test.

.. code:: bash

   aecmio_TEST_PATH="<LOCAL_PATH_TO_aec_TESTS>" test/testaec -t GeneralStateTests/stExample2 -- --filltests --checkstate

where the environmental variable ``aecmio_TEST_PATH`` should point to the directory where ``tests`` repository is checked out.  ``stExample2`` should be replaced with the name of the subdirectory you are working on.  ``--filltests`` option tells ``testaec`` to fill tests.  ``--checkstate`` tells ``testaec`` to check the final states against the ``expect`` fields.

Depending on your shell, there are various ways to set up ``aecmio_TEST_PATH`` environment variable.  For example, adding ``export aecmio_TEST_PATH=/path/to/tests`` to ``~/.bashrc`` might work for ``bash`` users.

``testaec`` with ``--filltests`` fills every test filler it finds. The command might modify existing test cases. After running ``testaec`` with ``--filltests``, try running ``git status`` in the ``tests`` directory. If ``git status`` indicates changes in unexpected files, that is an indication that the behavior of ``cpp-aecmio`` changed unexpectedly.

.. note::
   If ``testaec`` is looking for tests in the ``../../test/jsontests`` directory, 
...
For trying the filled test, in ``cpp-aecmio/build`` directory, run

.. code:: bash

   aecmio_TEST_PATH="../../tests" test/testaec -t GeneralStateTests/stExample2

Trying the Filled Test in Travis CI
+++++++++++++++++++++++++++++++++++

Moreover, for trying the filled test in ``Travis CI`` for ``aecmio/cpp-aecmio``, the new test cases need to exist in a branch in ``aecmio/tests``.   For this, ask somebody with a push permission to ``aecmio/tests``.

After that, enter ``cpp-aecmio/test/jsontests`` directory, and checkout the branch in ``aecmio/tests``.  Then go back to ``cpp-aecmio`` directory and perform ``git add test/jsontests`` followed by ``git commit``.

When you file this commit as a pull-request_ to ``aecmio/cpp-aecmio``, Travis CI should try the newly filled tests.
...
.. code::
  ...
  aecmio_TEST_PATH="../../tests" test/testaec -t GeneralStateTests/stExample2 -- --filltests --fillchain --checkstate

followed by

.. code::

  aecmio_TEST_PATH="../../tests" test/testaec -t GeneralStateTests/stExample2 -- --filltests --checkstate
...
Advanced: When testaec Takes Too Much Time
==========================================

Sometimes, especially when you are running BlockchainTests, ``testaec`` takes a lot of time.
...
``testaec`` has options to run tests selectively:
...
``--singletest`` option removes skipped tests from the final test file, when ``testaec`` is filling a BlockchainTest.
