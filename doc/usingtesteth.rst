...
aecmio cpp-client testaec tool for creation and execution of aecmio tests.

To run tests you should open folder (see also `Installing and building <https://github.com/aecmio/cpp-aecmio#building-from-source>`_)
...
By default `testaec` will look for the test repository cloned in cpp-aecmio submodule ``cpp-aecmio/test/jsontests`` assuming that the build folder is ``cpp-aecmio/build``

If environment variable ``aecmio_TEST_PATH`` is set in /etc/environment file, ``testaec`` will use path to the test repo from that variable. Example:

|    ``nano /etc/environment``
|    ``aecmio_TEST_PATH="/home/user/aecmio/tests"``

You could always override the test path for testaec using an option:

   ``./testaec -- --testpath "/path/to/the/tests"``
   
Note that --testpath option argument should be an absolute path.
For a brief help on testaec command options make sure to run 
...
To run a specific test case you could use parameter ``-t`` in the command line option:
    ...
    ``./testaec -t <TEST_SUITE>/<TEST_CASE>``

Or just the test suite:

   ``./testaec -t <TEST_SUITE>``
   
To run a specific test from the test case:

   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --singletest <TEST_NAME>``
...
https://github.com/aecmio/cpp-aecmio/blob/develop/libaecashseal/genesis/mainNetwork.cpp

If you need to debug a specific test on a specific network rules use this command:

   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --singletest <TEST_NAME> --singlenet <NET_NAME>``
...
This transaction has arrays <data>, <value>, <gasLimit>. So a single test execute transaction with different arguments taken from those arrays to test different conditions on the same pre state. To run a transaction from the GeneralStateTests with the specified arguments use: 
   ...
   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --singletest <TEST_NAME> --singlenet <NET_NAME> -d <DATA_INDEX> -g <GASLIMIT_INDEX> -v <VALUE_INDEX>``
...
``testaec`` has debugging options for getting a step by step execution log from the EVM. 
Use following options:

   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --vmtrace --verbosity 5``
   
``--vmtrace`` prints a step by step execution log from the EVM. Make sure that you've run cmake with -DVMTRACE=1 flag.

   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --jsontrace <CONFIG>``
   
An rpc maecod like, providing step by step debug in json format. The <CONFIG> is in json format like following: 

   ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --jsontrace '{ "disableStorage" : false, "disableMemory" : false, "disableStack" : false, "fullStorage" : true }' ``
   
Or just empty string to load default options.

    ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --jsontrace '' ``
...
You could specify some of the options in this json line or use an empty argument to load default options. Sometimes you might want to disable just the memory logs or the storage logs or both cause it could provide a lot lines to the final log.
    ...
    ``./testaec -t <TEST_SUITE>/<TEST_CASE> -- --statediff``
...
Tests are generated from test filler files located in the src folder of the test repo. Testaec will run the execution of a *Filler.json file and produce a final test in the repo.
``--filltests`` option will rerun test creation. .json files will be overwritten, hashes recalculated and a fresh build info will be added to the tests.  
    ...
    ``./testaec -t <TEST_SUITE>/<TEST_CASE> --filltests --checkstate --all``

If some test case has different results than it is expected to have (an expect section is specified in the *Filler.json file) then you will see an error in the cmd log. This error indicates that somaecing went different and the post state does not match the expect section. So you should check the test and it's expect section. Generating a test case and creating new tests is rather a whole new topic and it's described in more detail here: 

   https://github.com/aecmio/cpp-aecmio/blob/develop/doc/generating_tests.rst
