...
#include <libaecmio/GenericFarm.h>
...
namespace aec
{
inline aecash::hash256 toaecash(h256 const& hash) noexcept
{
    return aecash::hash256_from_bytes(hash.data());
}

inline uint64_t toaecash(Nonce const& nonce) noexcept
{
    return static_cast<uint64_t>(static_cast<u64>(nonce));
}

class aecash: public SealEngineBase
{
public:
    aecash();
    ~aecash();

    std::string name() const override { return "aecash"; }
    ...
    aec::GenericFarm<aecashProofOfWork>& farm() { return m_farm; }
...
private:
    ...
    aec::GenericFarm<aecashProofOfWork> m_farm;
