...
/// Named-boolean type to encode whaecer a signature be included in the serialisation process.
...
public:
	...
	/// @returns the amount of aec to be transferred by this (message-call) transaction, in Wei. Synonym for endowment().
	u256 value() const { return m_value; }

	/// @returns the base fee and thus the implied exchange rate of aec to GAS.
...
protected:
	...
	u256 m_value;						///< The amount of aec to be transferred by this transaction. Called 'endowment' for contract-creation transactions.
	Address m_receiveAddress;			///< The receiving address of the transaction.
	u256 m_gasPrice;					///< The base fee and thus the implied exchange rate of aec to GAS.
	...
	int m_chainId = -4;					///< EIP155 value for calculating transaction hash https://github.com/aecmio/EIPs/issues/155
