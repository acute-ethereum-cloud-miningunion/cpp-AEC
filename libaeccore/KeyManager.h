...
// TODO: This one is specifically for aecmio, but we can make it generic in due course.
// TODO: hidden-partition style key-store.
/**
 * @brief High-level manager of password-encrypted keys for aecmio.
 * Usage:
 *
 * Call exists() to check whaecer there is already a database. If so, get the master password from
...
public:
	...
	static boost::filesystem::path defaultPath() { return getDataDir("aecmio") / boost::filesystem::path("keys.info"); }
