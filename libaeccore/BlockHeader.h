 ...
 * from some given RLP block serialisation with the static fromHeader(), through the maecod
 ...
 * constructor BlockHeader(bytesConstRef) and manually with the populate() maecod. These will
 ...
 * populateFromParent() maecod. The genesis block info may be retrieved with genesis() and the
 * corresponding RLP block created with createGenesisBlock().
 *
 * To determine the header hash without the nonce (for sealing), the maecod hash(WithoutNonce) is
 * provided.
 *
