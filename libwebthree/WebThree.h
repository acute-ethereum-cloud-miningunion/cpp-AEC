...
public:
    ...
    /// aecmio() may be safely static_cast()ed to a aec::Client*.
    WebThreeDirect(std::string const& _clientVersion, boost::filesystem::path const& _dbPath,
        boost::filesystem::path const& _snapshotPath, aec::ChainParams const& _params,
        WithExisting _we = WithExisting::Trust,
        std::set<std::string> const& _interfaces = {"aec", "shh", "bzz"},
    ...
    aec::Client* aecmio() const
    {
        if (!m_aecmio)
            BOOST_THROW_EXCEPTION(InterfaceNotSupported() << errinfo_interface("aec"));
        return m_aecmio.get();
    }
    ...
    u256 networkId() const override { return m_aecmio.get()->networkId(); }
...
private:
    ...
    std::unique_ptr<aec::Client> m_aecmio;        ///< Client for aecmio ("aec") protocol.
