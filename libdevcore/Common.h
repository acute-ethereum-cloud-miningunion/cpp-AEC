...
public:
    secure_vector() {}
    secure_vector(secure_vector<T> const& /*_c*/) = default;  // See https://github.com/aecmio/libweb3core/pull/44
...
#define DEV_TIMED(S) for (::std::pair<::dev::TimerHelper, bool> __aec_t(S, true); __aec_t.second; __aec_t.second = false)
#define DEV_TIMED_SCOPE(S) ::dev::TimerHelper __aec_t(S)
#if defined(_WIN32)
...
#define DEV_TIMED_ABOVE(S, MS) for (::std::pair<::dev::TimerHelper, bool> __aec_t(::dev::TimerHelper(S, MS), true); __aec_t.second; __aec_t.second = false)
#define DEV_TIMED_SCOPE_ABOVE(S, MS) ::dev::TimerHelper __aec_t(S, MS)
#if defined(_WIN32)
