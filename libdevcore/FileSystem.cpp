...
static fs::path s_aecmioDatadir;
static fs::path s_aecmioIpcPath;

void dev::setDataDir(fs::path const& _dataDir)
{
	s_aecmioDatadir = _dataDir;
}

void dev::setIpcPath(fs::path const& _ipcDir)
{
	s_aecmioIpcPath = _ipcDir;
}

fs::path dev::getIpcPath()
{
	// Strip "gaec.ipc" suffix if provided.
	if (s_aecmioIpcPath.filename() == "gaec.ipc")
		return s_aecmioIpcPath.parent_path();
	else
		return s_aecmioIpcPath;
}
...
fs::path dev::getDataDir(string _prefix)
{
	if (_prefix.empty())
		_prefix = "aecmio";
	if (_prefix == "aecmio" && !s_aecmioDatadir.empty())
		return s_aecmioDatadir;
