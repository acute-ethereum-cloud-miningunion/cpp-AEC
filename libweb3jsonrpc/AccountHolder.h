 ...
 * @authors:
 *   Christian R <c@aecdev.com>
 *   Lefteris Karapetsas <lefteris@aecdev.com>
 * @date 2015
 */
...
public:
	...
	virtual TransactionNotification authenticate(dev::aec::TransactionSkeleton const& _t) = 0;
	...
	void queueTransaction(aec::TransactionSkeleton const& _transaction);

	std::vector<aec::TransactionSkeleton> const& queuedTransactions(int _id) const;
...
private:
	using TransactionQueue = std::vector<aec::TransactionSkeleton>;
...
public:
	...
	TransactionNotification authenticate(dev::aec::TransactionSkeleton const& _t) override;
